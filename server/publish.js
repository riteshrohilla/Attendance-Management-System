...
Meteor.publish("get.login.student", () => {
  return Student.find({ accountId: Meteor.userId() });
});
Meteor.publish("get.attendance", id => {
  return Attendance.find({ classId: id });
});
Meteor.publish("get.student.attendance", () => {
  return Attendance.find({ studentId: Meteor.userId()});
});
